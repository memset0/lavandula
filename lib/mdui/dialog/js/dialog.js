...
lavandula.Dialog = (function () {
  ...
  var readjust = function () {
    ...
    var $dialogTitle = $dialog.children('.lavandula-dialog-title');
    var $dialogContent = $dialog.children('.lavandula-dialog-content');
    var $dialogActions = $dialog.children('.lavandula-dialog-actions');
    ...
    // 调整 lavandula-dialog-content 的高度
  ...
  var hashchangeEvent = function () {
    if (location.hash.substring(1).indexOf('lavandula-dialog') < 0) {
  ...
  var overlayClick = function (e) {
    if ($(e.target).hasClass('lavandula-overlay') && currentInst) {
      currentInst.close();
    }
  };
  ...
  function Dialog(selector, opts) {
    ...
    var oldInst = _this.$dialog.data('lavandula.dialog');
    if (oldInst) {
      return oldInst;
    }
    ...
    // 绑定取消按钮事件
    _this.$dialog.find('[lavandula-dialog-cancel]').each(function () {
    ...
    // 绑定确认按钮事件
    _this.$dialog.find('[lavandula-dialog-confirm]').each(function () {
      $(this).on('click', function () {
    ...
    // 绑定关闭按钮事件
    _this.$dialog.find('[lavandula-dialog-close]').each(function () {
      $(this).on('click', function () {
        _this.close();
      });
    });
  ...
  var transitionEnd = function (inst) {
    if (inst.$dialog.hasClass('lavandula-dialog-open')) {
      inst.state = 'opened';
  ...
  Dialog.prototype._doOpen = function () {
    ...
    _this.$dialog
      .addClass('lavandula-dialog-open')
    ...
    if (_this.options.history) {
      // 如果 hash 中原来就有 lavandula-dialog，先删除，避免后退历史纪录后仍然有 lavandula-dialog 导致无法关闭
      // 包括 lavandula-dialog 和 &lavandula-dialog 和 ?lavandula-dialog
      var hash = location.hash.substring(1);
      if (hash.indexOf('lavandula-dialog') > -1) {
        hash = hash.replace(/[&?]?lavandula-dialog/g, '');
      }

      // 后退按钮关闭对话框
      if (hash) {
        location.hash = hash + (hash.indexOf('?') > -1 ? '&' : '?') + 'lavandula-dialog';
      } else {
        location.hash = 'lavandula-dialog';
      }
  ...
  Dialog.prototype.close = function () {
    ...
    setTimeout(function () {
      ...
      _this.$dialog
        .removeClass('lavandula-dialog-open')
        .transitionEnd(function () {
          transitionEnd(_this);
        });
  ...
  Dialog.prototype.destroy = function () {
    ...
    _this.$dialog.removeData('lavandula.dialog');
