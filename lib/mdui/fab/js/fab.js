...
lavandula.Fab = (function () {
  ...
  function Fab(selector, opts) {
    ...
    var oldInst = _this.$fab.data('lavandula.fab');
    if (oldInst) {
      return oldInst;
    }
    ...
    _this.$btn = _this.$fab.find('.lavandula-fab');
    _this.$dial = _this.$fab.find('.lavandula-fab-dial');
    _this.$dialBtns = _this.$dial.find('.lavandula-fab');
    ...
    $document.on(TouchHandler.start, function (e) {
      if (!$(e.target).parents('.lavandula-fab-wrapper').length) {
        _this.close();
      }
    });
  }
  ...
  Fab.prototype.open = function () {
    ...
    _this.$dial
      .css('height', 'auto')
      .addClass('lavandula-fab-dial-show');

    // 如果按钮中存在 .lavandula-fab-opened 的图标，则进行图标切换
    if (_this.$btn.find('.lavandula-fab-opened').length) {
      _this.$btn.addClass('lavandula-fab-opened');
    }
    ...
    _this.$dialBtns.eq(0).transitionEnd(function () {
      if (_this.$btn.hasClass('lavandula-fab-opened')) {
        _this.state = 'opened';
  ...
  Fab.prototype.close = function () {
    ...
    _this.$dial.removeClass('lavandula-fab-dial-show');
    _this.$btn.removeClass('lavandula-fab-opened');
    _this.state = 'closing';
  ...
  Fab.prototype.show = function () {
    this.$fab.removeClass('lavandula-fab-hide');
  };
  ...
  Fab.prototype.hide = function () {
    this.$fab.addClass('lavandula-fab-hide');
  };
