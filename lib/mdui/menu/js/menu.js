...
lavandula.Menu = (function () {
  ...
  var readjustSubmenu = function ($submenu) {
    var $item = $submenu.parent('.lavandula-menu-item');
  ...
  var openSubMenu = function ($submenu) {
    ...
    $submenu
      .addClass('lavandula-menu-open')
      .parent('.lavandula-menu-item')
      .addClass('lavandula-menu-item-active');
  };
  ...
  var closeSubMenu = function ($submenu) {
    // 关闭子菜单
    $submenu
      .removeClass('lavandula-menu-open')
      .addClass('lavandula-menu-closing')
      .transitionEnd(function () {
        $submenu.removeClass('lavandula-menu-closing');
      })

      // 移除激活状态的样式
      .parent('.lavandula-menu-item')
      .removeClass('lavandula-menu-item-active');

    // 循环关闭嵌套的子菜单
    $submenu.find('.lavandula-menu').each(function () {
      var $subSubmenu = $(this);
      $subSubmenu
        .removeClass('lavandula-menu-open')
        .addClass('lavandula-menu-closing')
        .transitionEnd(function () {
          $subSubmenu.removeClass('lavandula-menu-closing');
        })
        .parent('.lavandula-menu-item')
        .removeClass('lavandula-menu-item-active');
    });
  ...
  var toggleSubMenu = function ($submenu) {
    if ($submenu.hasClass('lavandula-menu-open')) {
  ...
  var bindSubMenuEvent = function (inst) {
    // 点击打开子菜单
    inst.$menu.on('click', '.lavandula-menu-item', function (e) {
      var $this = $(this);
      var $target = $(e.target);
      ...
      if ($target.is('.lavandula-menu') || $target.is('.lavandula-divider')) {
        return;
      }

      // 阻止冒泡，点击菜单项时只在最后一级的 lavandula-menu-item 上生效，不向上冒泡
      if (!$target.parents('.lavandula-menu-item').eq(0).is($this)) {
        return;
      }

      // 当前菜单的子菜单
      var $submenu = $this.children('.lavandula-menu');

      // 先关闭除当前子菜单外的所有同级子菜单
      $this.parent('.lavandula-menu').children('.lavandula-menu-item').each(function () {
        var $tmpSubmenu = $(this).children('.lavandula-menu');
        if (
          $tmpSubmenu.length &&
    ...
    if (inst.options.subMenuTrigger === 'hover') {
      ...
      inst.$menu.on('mouseover mouseout', '.lavandula-menu-item', function (e) {
        var $this = $(this);
        ...
        if (eventType === 'mouseover') {
          if ($submenu.length) {
            ...
            var tmpClose = $submenu.data('timeoutClose.lavandula.menu');
            if (tmpClose) {
            ...
            $submenu.data('timeoutOpen.lavandula.menu', timeout);
          }
        }
        ...
        // 鼠标移出菜单项时，关闭菜单项下的子菜单
          ...
          if ($submenu.length) {
            ...
            var tmpOpen = $submenu.data('timeoutOpen.lavandula.menu');
            if (tmpOpen) {
            ...
            $submenu.data('timeoutClose.lavandula.menu', timeout);
          }
        }
      });
    }
  ...
  function Menu(anchorSelector, menuSelector, opts) {
    ...
    var oldInst = _this.$anchor.data('lavandula.menu');
    if (oldInst) {
      return oldInst;
    }
    ...
    // 是否是级联菜单
    _this.isCascade = _this.$menu.hasClass('lavandula-menu-cascade');
    ...
    // 点击不含子菜单的菜单条目关闭菜单
    $document.on('click', '.lavandula-menu-item', function (e) {
      var $this = $(this);
      if (!$this.find('.lavandula-menu').length && $this.attr('disabled') === null) {
        _this.close();
      }
    });
  ...
  var transitionEnd = function (inst) {
    inst.$menu.removeClass('lavandula-menu-closing');
  ...
  Menu.prototype.close = function () {
    ...
    // 菜单开始关闭时，关闭所有子菜单
    _this.$menu.find('.lavandula-menu').each(function () {
      closeSubMenu($(this));
    });
