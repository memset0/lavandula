...
lavandula.Tab = (function () {
  ...
  function Tab(selector, opts) {
    ...
    var oldInst = _this.$tab.data('lavandula.tab');
    if (oldInst) {
      return oldInst;
    }
    ...
    _this.$indicator = $('<div class="lavandula-tab-indicator"></div>').appendTo(_this.$tab);
    ...
    // 含 lavandula-tab-active 的元素默认激活
    if (_this.activeIndex === false) {
      _this.$tabs.each(function (i, tab) {
        if ($(tab).hasClass('lavandula-tab-active')) {
          _this.activeIndex = i;
          return false;
        }
      });
    ...
    _this.$tabs.each(function (i, tab) {
      ...
      if (i === _this.activeIndex && !isDisabled($tab)) {
        if (!$tab.hasClass('lavandula-tab-active')) {
          ...
          $tab.addClass('lavandula-tab-active');
        }

        $(targetId).show();
        _this._setIndicatorPosition();
      } else {
        $tab.removeClass('lavandula-tab-active');
        $(targetId).hide();
      }
    });
  };
