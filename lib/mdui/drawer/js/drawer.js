...
lavandula.Drawer = (function () {
  ...
  function Drawer(selector, opts) {
    ...
    var oldInst = _this.$drawer.data('lavandula.drawer');
    if (oldInst) {
      return oldInst;
    }
    ...
    _this.position = _this.$drawer.hasClass('lavandula-drawer-right') ? 'right' : 'left';

    if (_this.$drawer.hasClass('lavandula-drawer-close')) {
      _this.state = 'closed';
    } else if (_this.$drawer.hasClass('lavandula-drawer-open')) {
      _this.state = 'opened';
    } else if (isDesktop()) {
      _this.state = 'opened';
    } else {
    ...
    $window.on('resize', $.throttle(function () {
      ...
      // 由桌面切换到手机平板时。如果抽屉栏是打开着的且没有遮罩层，则关闭抽屉栏
        ...
        if (!_this.overlay && _this.state === 'opened') {
          // 抽屉栏处于强制打开状态，添加遮罩
          if (_this.$drawer.hasClass('lavandula-drawer-open')) {
            $.showOverlay();
            _this.overlay = true;
            $.lockScreen();

            $('.lavandula-overlay').one('click', function () {
              _this.close();
            });
          } else {
            _this.state = 'closed';
    ...
    // 绑定关闭按钮事件
    _this.$drawer.find('[lavandula-drawer-close]').each(function () {
  ...
  var transitionEnd = function (inst) {
    if (inst.$drawer.hasClass('lavandula-drawer-open')) {
      inst.state = 'opened';
  ...
  Drawer.prototype.open = function () {
    ...
    if (!_this.options.overlay) {
      $('body').addClass('lavandula-drawer-body-' + _this.position);
    }

    _this.$drawer
      .removeClass('lavandula-drawer-close')
      .addClass('lavandula-drawer-open')
  ...
  Drawer.prototype.close = function () {
    ...
    if (!_this.options.overlay) {
      $('body').removeClass('lavandula-drawer-body-' + _this.position);
    }

    _this.$drawer
      .addClass('lavandula-drawer-close')
      .removeClass('lavandula-drawer-open')
      .transitionEnd(function () {
        transitionEnd(_this);
      });
