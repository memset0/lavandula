...
(function () {
   ...
   *    lavandula.mutation(selector, apiInit);
   *    mutation 会执行 $(selector).each(apiInit)
   *
   * 2. 突变时, 再次执行代理的初始化函数
   *    lavandula.mutation()        等价 $(document).mutation()
   *    $(selector).mutation() 在 selector 节点内进行 API 初始化
   *
   * 原理:
   *
   *    mutation 执行了 $().data('lavandula.mutation', [selector]).
  ...
  function mutation(selector, apiInit, that, i, item) {
    var $this = $(that);
    var m = $this.data('lavandula.mutation');

    if (!m) {
      m = [];
      $this.data('lavandula.mutation', m);
    }
  ...
  lavandula.mutation = function (selector, apiInit) {
