...
(function () {
  ...
  var closeOnOutsideClick = function (e) {
    var $target = $(e.target);
    if (!$target.hasClass('lavandula-snackbar') && !$target.parents('.lavandula-snackbar').length) {
      currentInst.close();
    }
  };
  ...
  function Snackbar(message, opts) {
    ...
    if (
      ...
      buttonColorClass = 'lavandula-text-color-' + _this.options.buttonColor;
    }

    // 添加 HTML
    _this.$snackbar = $(
      '<div class="lavandula-snackbar">' +
        '<div class="lavandula-snackbar-text">' +
          _this.message +
        '</div>' +
        (_this.options.buttonText ?
          ('<a href="javascript:void(0)" ' +
          'class="lavandula-snackbar-action lavandula-btn lavandula-ripple lavandula-ripple-white ' +
            buttonColorClass + '" ' +
            buttonColorStyle + '>' +
            _this.options.buttonText +
          '</a>') :
    ...
    _this.$snackbar
      .reflow()
      .addClass('lavandula-snackbar-' + _this.options.position);
  }
    ...
    _this.$snackbar
      .transitionEnd(function () {
        ...
        if (_this.options.buttonText) {
          _this.$snackbar
            .find('.lavandula-snackbar-action')
            .on('click', function () {
        ...
        _this.$snackbar.on('click', function (e) {
          if (!$(e.target).hasClass('lavandula-snackbar-action')) {
   ...
   * @param opts
   */
  lavandula.snackbar = function (message, opts) {
