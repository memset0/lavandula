...
(function () {
  ...
  var checkboxHTML = function (tag) {
    return '<' + tag + ' class="lavandula-table-cell-checkbox">' +
  ...
  Table.prototype.init = function () {
    ...
    _this.$tdCheckboxs = $();
    _this.selectable = _this.$table.hasClass('lavandula-table-selectable');
    _this.selectedRow = 0;
  ...
  Table.prototype._updateTdCheckbox = function () {
    ...
    _this.$tdRows.each(function () {
      ...
      // 移除旧的 checkbox
      $tdRow.find('.lavandula-table-cell-checkbox').remove();
      ...
      // 默认选中的行
      if ($tdRow.hasClass('lavandula-table-row-selected')) {
      ...
      $checkbox.on('change', function () {
        if ($checkbox[0].checked) {
          $tdRow.addClass('lavandula-table-row-selected');
          _this.selectedRow++;
        } else {
          $tdRow.removeClass('lavandula-table-row-selected');
          _this.selectedRow--;
        }
  ...
  Table.prototype._updateThCheckbox = function () {
    ...
    // 移除旧的 checkbox
    _this.$thRow.find('.lavandula-table-cell-checkbox').remove();
    ...
    _this.$thCheckbox = $(checkboxHTML('th'))
      ...
      .on('change', function () {
        ...
        _this.$tdRows.each(function (i, row) {
          $(row)[isCheckedAll ? 'addClass' : 'removeClass']('lavandula-table-row-selected');
        });
      });
  };
  ...
  Table.prototype._updateNumericCol = function () {
    ...
    _this.$thRow.find('th').each(function (i, th) {
      ...
      _this.$tdRows.each(function () {
        $tdRow = $(this);
        var method = $th.hasClass('lavandula-table-col-numeric') ? 'addClass' : 'removeClass';
        $tdRow.find('td').eq(i)[method]('lavandula-table-col-numeric');
      });
    });
  };
   ...
   * 初始化表格
   */
  lavandula.mutation('.lavandula-table', function () {
    var $table = $(this);
    if (!$table.data('lavandula.table')) {
      $table.data('lavandula.table', new Table($table));
    }
  });
   ...
   * 更新表格
   */
  lavandula.updateTables = function () {
    $(arguments.length ? arguments[0] : '.lavandula-table').each(function () {
      var $table = $(this);
      var inst = $table.data('lavandula.table');
